...
Features:
    ...
    - 6 shape classes + floor background (7 segmentation classes total)
    - Class-independent random per-instance colours (no colour->label shortcut)
    - Bounding boxes (gt_boxes) for 3D detection — shapes only, NOT the floor
    - Rotation and scale augmentation

Segmentation classes (7):
    0: cube, 1: sphere, 2: cylinder, 3: cone, 4: pyramid, 5: torus, 6: floor
Detection classes (6): the shapes 0-5 only — the floor plane gets no box.
...
# Shape classes (6 detectable shapes) + the floor background plane.
# Only the shapes get detection boxes; the floor is segmentation-only.
SHAPE_CLASSES = ['cube', 'sphere', 'cylinder', 'cone', 'pyramid', 'torus']
SHAPE_LABELS = {name: i for i, name in enumerate(SHAPE_CLASSES)}
FLOOR_LABEL = 6  # background floor plane — segmentation label only, no box
...
def generate_floor(floor_size, num_points=3000):
    """Generate the flat floor plane at Z=0 (background, not a detected object)."""
    w_width, w_height = floor_size

    floor_x = np.random.uniform(-w_width/2, w_width/2, num_points)
    floor_y = np.random.uniform(-w_height/2, w_height/2, num_points)
    floor_z = np.zeros(num_points)

    return np.column_stack([floor_x, floor_y, floor_z]).astype(np.float32)


def random_instance_color(num_points, noise_level=20):
    """RGB colours for one shape instance — a CLASS-INDEPENDENT random base
    colour plus per-point noise.

    Colour deliberately carries NO class signal: a cube in one scene may be
    blue, in the next scene red. This forces the segmentation head to learn
    GEOMETRY rather than trivially reading the colour channel as the label
    (the old per-class palette made 99.99% mIoU a colour lookup, not real
    learning). Detection / SSL benefit only shows when the task is honest.
    """
    base = np.random.randint(40, 216, 3)
    noise = np.random.randint(-noise_level, noise_level + 1, (num_points, 3))
    return np.clip(base + noise, 0, 255).astype(np.float32)


def generate_scene(scene_idx, out_dir, split, floor_size=(100, 100),
                   points_per_shape=2000, floor_points=3000):
    """Generate a single scene with 3-5 floating shapes above a floor plane."""
    ...
    for i, shape_name in enumerate(selected_shapes):
        ...
        # Height above floor: 15-25 units
        height_above_floor = np.random.uniform(15, 25)

        generator = SHAPE_GENERATORS[shape_name]
        center_pos = [offset_x, offset_y, height_above_floor + 8]
        ...
        all_shape_colors.append(random_instance_color(len(pts)))
    ...
    # Generate the floor plane (background — segmentation only, no box)
    floor_pts = generate_floor(floor_size, floor_points)
    floor_labels = np.full(len(floor_pts), FLOOR_LABEL, dtype=np.int32)
    floor_colors = random_instance_color(len(floor_pts))

    # Combine all points
    all_points = np.vstack(all_shape_points + [floor_pts])
    all_labels = np.concatenate(all_shape_labels + [floor_labels])
    all_colors = np.vstack(all_shape_colors + [floor_colors])
    ...
    # Colours were assigned per-instance above (class-independent)
    colors = all_colors
...
def main():
    ...
    # Save classes.json
    import json
    class_names = SHAPE_CLASSES + ['floor']
