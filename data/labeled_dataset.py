...
class CustomDataset(Dataset):
    ...
    def _load_npy(self, scene_path):
        ...
        try:
            ...
            # Optional: clip points to a configured range. The mask is APPLIED
            # AFTER features + segment are loaded so all per-point arrays stay
            # the same length (clipping coord alone here would make the later
            # np.concatenate of coord+colour fail on mismatched row counts).
            range_mask = None
            if hasattr(self.cfg, 'DETECTION_CONFIG'):
                 pc_range = self.cfg.DETECTION_CONFIG.get('POINT_CLOUD_RANGE')
                 if pc_range:
                     m = (coord[:, 0] >= pc_range[0]) & (coord[:, 0] <= pc_range[3]) & \
            ...
            if self.load_seg:
                ...
                if os.path.exists(seg_path):
                    ...
                    # apply config label remapping (parity with the PLY path)
            ...
            # Apply the point-cloud-range mask to every per-point array at once
            if range_mask is not None:
                coord = coord[range_mask]
                features = features[range_mask]
                segment = segment[range_mask]
    ...
    def calculate_class_weights(self, num_classes):
        """Calculate balanced class weights with content-aware caching."""
        import json
        sig = self._cache_signature("segment.npy")
        cache_key = {"num_classes": int(num_classes), **sig}
        ...
        if os.path.exists(cache_path):
            try:
                ...
                if (isinstance(cached, dict) and cached.get("key") == cache_key
                        and isinstance(cached.get("value"), list)
                        and len(cached["value"]) == num_classes):
        ...
        # Save cache (with content signature for safe reuse)
        try:
            with open(cache_path, 'w') as f:
                json.dump({"key": cache_key, "value": weights_list}, f)
        except Exception:
            pass

        return weights_list

    def _cache_signature(self, label_filename):
        """Cheap dataset fingerprint = (n_scenes, sum of label-file mtimes).

        Invalidates the cache when scenes are added/removed or label files are
        re-saved — without re-reading their contents.
        """
        total_mtime = 0.0
        for scene in self.scenes:
            p = (scene.replace(".ply", label_filename)
                 if scene.endswith(".ply")
                 else os.path.join(scene, label_filename))
            if os.path.exists(p):
                total_mtime += os.path.getmtime(p)
        return {"n_scenes": len(self.scenes),
                "mtime_sum": round(total_mtime, 3)}


    def calculate_mean_sizes(self, num_classes):
        """Calculate mean box sizes per class with content-aware caching."""
        import json

        # cache signature = (num_classes, scene count, sum of gt_boxes mtimes)
        # — invalidates the cache whenever the data changes (scenes added /
        # removed / boxes re-saved), which the old class-count-only key missed
        sig = self._cache_signature("gt_boxes.npy" if self.data_format == "npy"
                                     else "_gt_boxes.npy")
        cache_key = {"num_classes": int(num_classes), **sig}
        ...
        if os.path.exists(cache_path):
            try:
                ...
                if (isinstance(cached, dict) and cached.get("key") == cache_key
                    ...
                    return cached["value"]
                print("Cache stale (data or class-count changed), recalculating...")
            except Exception as e:
        ...
        mean_sizes = []
        for i in range(num_classes):
            # gt_boxes label column and the detection box-coder's predicted
            # class are BOTH 0-based (class 0 = first object class). mean_sizes
            # is indexed by that 0-based class, so look up `sums[i]` directly.
            # (A previous `i + 1` here shifted every anchor to the next class,
            # e.g. the pyramid received the torus anchor -> pyramid undetected.)
            if i in sums and counts[i] > 0:
                mean_sizes.append((sums[i] / counts[i]).tolist())
            else:
                mean_sizes.append([1.0, 1.0, 1.0])  # class absent from data
        
        # Save cache (with content signature for safe reuse)
        try:
            with open(cache_path, 'w') as f:
                json.dump({"key": cache_key, "value": mean_sizes}, f)
