...
def evaluate_detection(model, loader, num_det, device,
    ...
    Per scene:
      1. Forward the model.
      2. Per-class BEV NMS dedupes the per-point box flood
         (`Custom.postprocess.filter_and_nms` with `conf_thresh=0.0` — NO hard
         confidence cut, because mAP integrates precision/recall over all
         scores).
      3. Feed survivors + GT into `metrics.detection_metrics.DetectionMetrics`,
         which computes per-class AP via the 11-point / area-under-PR-curve
         scheme at each IoU threshold using full rotated-3D IoU.
    ...
    for batch in loader:
        ...
        # raw post-processed boxes; per-class NMS (no conf filter — mAP
        # integrates over scores).
    ...
    # Always surface a per-class AP@0.5 list for downstream reporting.
    per_class_ap50 = []
    for c in range(num_det):
        name = (class_names[c] if class_names and c < len(class_names)
                else f"class_{c}")
        per_class_ap50.append(float(results.get(f"AP_{name}@0.5", 0.0)))
    results["per_class_AP@0.5"] = per_class_ap50
    return results
