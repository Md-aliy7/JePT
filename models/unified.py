...
def create_unified_model(
    ...
    # Optionally load a JEPA-pretrained backbone before fine-tuning
    pretrained = getattr(cfg, 'PRETRAINED_CKPT', None)
    if pretrained:
        load_pretrained_backbone(model, pretrained, verbose=True)
...
# ============================================================================
# JEPA PRETRAINED WEIGHT LOADING
# ============================================================================

def load_pretrained_backbone(model, ckpt_path, verbose=True):
    """Load a JEPA-pretrained LitePT backbone into a downstream unified model.

    The JePT pretraining checkpoint stores both encoders. Downstream transfer
    uses the **EMA teacher** (target encoder) — the I-JEPA / DINO / data2vec
    convention: the moving-average encoder is a weight-space ensemble, smoother
    and consistently a better downstream initialisation than the gradient-
    trained student. Older checkpoints without a stored teacher fall back to the
    student. Either way the keys (``embedding.*``, ``enc.*``, ``dec.*``) match
    the ``LitePT`` instance(s) inside the downstream model exactly:

      * ``LitePTUnifiedCustom``  -> single ``backbone``  (full transfer)
      * ``LitePTDualPathUnified``-> ``seg_backbone`` (full transfer, multi-stage)
                                    and ``det_backbone`` (PARTIAL — the detection
                                    branch is single-stage, so only the input
                                    embedding overlaps; loaded with strict=False)

    Heads (`seg_head`, `det_head`), the decoder where shapes differ, and
    `log_vars` keep their random init. Loading is reported and shape-checked so a
    silent no-op is impossible.

    Args:
        model:     a `LitePTUnifiedCustom` or `LitePTDualPathUnified`.
        ckpt_path: path to a JePT pretraining checkpoint.
        verbose:   print a per-backbone matched/total summary.
    Returns:
        dict mapping backbone name -> (matched_keys, total_keys).
    """
    import os
    ...
    # Prefer the EMA teacher (target encoder). Its state lives under the EMA
    # wrapper, so strip the 'ema_model.' prefix (the 'step' buffer is dropped).
    teacher_sd = ckpt.get('teacher')
    if isinstance(teacher_sd, dict) and any(
            k.startswith('ema_model.') for k in teacher_sd):
        src_sd = {k[len('ema_model.'):]: v for k, v in teacher_sd.items()
                  if k.startswith('ema_model.')}
        src_name = 'teacher (EMA)'
    else:
        src_sd = ckpt.get('student', ckpt.get('model_state_dict', ckpt))
        src_name = 'student'

    targets = [name for name in ('backbone', 'seg_backbone', 'det_backbone')
    ...
    for prefix in targets:
        total = sum(1 for k in model_sd if k.startswith(prefix + '.'))
        matched = 0
        for k, v in src_sd.items():
            ...
            if mk in model_sd and model_sd[mk].shape == v.shape:
                new_sd[mk] = v
                matched += 1
        report[prefix] = (matched, total)

    missing = model.load_state_dict({**model_sd, **new_sd}, strict=False)
    if verbose:
        print(f"[load_pretrained_backbone] from {ckpt_path}  (encoder: {src_name})")
        # surface the pretraining context so a silent stage-mismatch is visible:
        # GRID_SIZE / INPUT_CHANNELS / MODEL_VARIANT must match between stages
        pv, pg, pc = (ckpt.get('variant'), ckpt.get('grid_size'),
                      ckpt.get('input_channels'))
        if pv is not None or pg is not None or pc is not None:
            print(f"  pretrained with: variant={pv}  grid_size={pg}  "
                  f"input_channels={pc}  (fine-tuning config MUST match these)")
        for prefix, (matched, total) in report.items():
            kind = 'FULL' if matched >= total * 0.9 else 'PARTIAL'
            print(f"  - {prefix:<13s}: {matched}/{total} tensors loaded [{kind}]")
        total_matched = sum(m for m, _ in report.values())
        if total_matched == 0:
            raise RuntimeError(
                "load_pretrained_backbone matched 0 tensors — checkpoint / "
